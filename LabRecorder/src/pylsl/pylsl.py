...
def pull_sample(self,timeout=FOREVER,sample=None):
    ...
    if type(timeout) is list:
        ...
        timestamp = self.do_pull_sample(self.obj, self.sample, self.channel_count, c_double(timeout),
                                        byref(errcode))
        handle_error(errcode)
    if timestamp:
...
def pull_chunk(self,timeout=0.0,max_samples=1024):
    ...
    if not max_samples in self.buffers:
        self.buffers[max_samples] = (cast((self.value_type*max_values)(),POINTER(self.value_type)),cast((c_double*max_samples)(),
                                                                                                        POINTER(c_double)))
